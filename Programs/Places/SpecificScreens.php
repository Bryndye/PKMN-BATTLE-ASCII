...
function chooseFirstPokemon(){
    ...
    Display_Game::messageBoiteDialogue('Choose your first Pokemon : 
...
function cinematicPresentation(){
    ...
    Display_Game::messageBoiteDialogue("Hello, i'm Prof. Twig and welcome to the world of Pokemon!", -1);
    Display_Game::messageBoiteDialogue("Let me show you what a pokemon is.", -1);
    ...
    Display_Game::messageBoiteDialogue("Here's Pikachu!", -1);
    Display_Game::messageBoiteDialogue("He's an electric type. You'll need Pokemons to do Pokemon battles. You can meet him later on your journey.", -1);
    Display::clearSprite($posSprite);
    Display::drawSprite(getSprites('trainer'), $posSprite);
    Display_Game::messageBoiteDialogue("By the way, what is your name?", -1);

    Display_Game::messageBoiteDialogue("'To select/ choose an action, write your answer under this box.'", -1);
    saveMainManager();
    $save = getSave(Parameters::getSavePath('myGame'));
    Display_Game::messageBoiteDialogue("Hi ". $save['name'].". Let me introduce you the rules.", -1);
    Display_Game::messageBoiteDialogue("But this time, it will be different. You have ".Parameters::getIndexFloorMaxOriginal()." battles to win.", -1);
    Display_Game::messageBoiteDialogue("You loose, you restart. Everytime you can choose a new starter Pokemon.", -1);
    Display_Game::messageBoiteDialogue("You can still buy items that can help you on your journey, but be careful with your money!", -1);
    Display_Game::messageBoiteDialogue("Capture Pokemons to grow up your team! You can only stock 6 Pokemons with you. If you capture one, you must replace it with a member of your team.", -1);
    Display_Game::messageBoiteDialogue("Do you see the difference? Of course you do!", -1);
    Display_Game::messageBoiteDialogue("I almost forgot! Here's a Pokedex that might help. When you see a Pokemon, It records Pokemon data .", -1);
    Display_Game::messageBoiteDialogue("And id you see my nephew, tell him his grandfather's looking for him.", -1);
    Display_Game::messageBoiteDialogue("So, you are ready. Good luck!", -1);
}

function cinematicLeagueEnding(&$save){
     Display::clearGameScreen();
    $name = getDataFromSave('name', Parameters::getSavePath('myGame'));
    Display_Game::messageBoiteDialogue('Congratulations! You beat the league Pokemon!', -1);
    ...
    foreach($save['Team'] as $pkmn){
        ...
        Display_Game::messageBoiteDialogue('Bravo to '.ucfirst($pkmn['Name']).'!');
    ...
    Display_Game::messageBoiteDialogue('Bravo '.$name.' and your team to defeat the league!', -1);
    Display_Game::messageBoiteDialogue("But it's not over!", -1);
    Display_Game::messageBoiteDialogue("We've heard that a very strong pokemon has appeared.", -1);
    Display_Game::messageBoiteDialogue("Are you ready? Go for it!", -1);
}
...
function cinematicEnding(&$save){
    ...
    if($win == 0){
        Display_Game::messageBoiteDialogue('New Pokemons have apparently appeared. New birds according to trainers.', -1);
    }
    else if ($win < 5){
        Display_Game::messageBoiteDialogue('You unlock new floors for the next time with new trainers and Pokemons.', -1);
    }
    Display_Game::messageBoiteDialogue("Your adventure isn't over yet. You must catch all the Pokemon!", -1);
    Display_Game::messageBoiteDialogue("See you later.", -1);
...
function screenLose(){
    ...
    Display_Game::messageBoiteDialogue('You lost at '. $floor . ' floor...', -1);
    deleteSave(Parameters::getSavePath('save'));
}
?>